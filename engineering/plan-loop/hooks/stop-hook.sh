...
main() {
  ...
  case "$PHASE" in
    draft)      INSTRUCTIONS="Phase: Draft (round ${ROUND}). Continue drafting the plan. Use the /make-plan discovery protocol if this is round 1, or revise based on prior round's FIX findings." ;;
    self-audit) INSTRUCTIONS="Phase: Self-Audit (round ${ROUND}). Spawn the review sub-agent to audit the plan for structural issues using the triage rubric severity definitions." ;;
    codex-audit) INSTRUCTIONS="Phase: Codex Audit (round ${ROUND}). Run \`codex exec --sandbox read-only\` with the plan audit prompt and assembled context." ;;
    triage)     INSTRUCTIONS="Phase: Triage (round ${ROUND}). Triage each Codex finding using mandatory context rebuttal (FIX/DISMISS/SCOPE-OUT with evidence). Apply round-aware severity threshold." ;;
    fix)        INSTRUCTIONS="Phase: Fix (round ${ROUND}). Apply all ABOVE-threshold FIX findings to the plan. Once all fixes are applied, increment the round with 'update-state.sh round $((ROUND + 1))' and proceed to Phase 2 (Self-Audit)." ;;
    *) approve; return ;;
  esac
...
Read .claude/plan-loop.local.json to confirm current round/phase, then continue the plan-loop skill. Update the state file at each phase boundary (the SKILL.md has the commands). To cancel, say 'cancel the plan loop'."
  ...
  SYSTEM_MSG="Plan loop block ${NEW_BLOCKS}/20 | Round ${ROUND}, phase: ${PHASE} | ${PLAN_NAME}"
